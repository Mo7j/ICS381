...
if __name__ == "__main__":
    ...
    clipped_monsters_example = GridHunterProblem(initial_agent_info=(0, 4, 'west', 10),
                                                 N=5,
                                                 monster_coords=[(0,0), (4,4), (0,4), (4,0)])
    example_state = (4, 2, 'north', 11, 2, False, False)
    print(f'For GridHunterProblem. With N=5, and monster_coords=[(2,2), (4,4), (0,4), (4,2)].')
    ...
    example_ghproblem = GridHunterProblem(initial_agent_info=(0, 4, 'west', 10),
                                          N=5, 
    ...
    actions_available = example_ghproblem.actions(state=example_state)
    ...
    next_state = example_ghproblem.result(state=example_state, action='move-forward')
    print(next_state)
    print("agent location should be updated to (4, 3).")
    print('________________________________________________________________________')
    
    # shoot-arrow mechanics
    example_state = (0, 4, 'west', 10, 3, False, False)
    print(f'For GridHunterProblem. From state: {example_state}. Taking action: {"shoot-arrow"}')
    next_state = example_ghproblem.result(state=example_state, action='shoot-arrow')
    print(next_state)
    print("agent should have eliminated monster at (0,1). So first boolean should be True.")
    print('________________________________________________________________________')
    
    example_state = (0, 2, 'south', 10, 1, False, False)
    print(f'For GridHunterProblem. From state: {example_state}. Taking action: {"shoot-arrow"}')
    next_state = example_ghproblem.result(state=example_state, action='shoot-arrow')
    print(next_state)
    print("agent should have eliminated monster at (2,2). So second boolean should be True.")
    print('________________________________________________________________________')
    
    # turning mechanics
    example_state = (4, 0, 'east', 10, 0, False, False)
    print(f'For GridHunterProblem. From state: {example_state}. Taking action: {"turn-left"}')
    next_state = example_ghproblem.result(state=example_state, action='turn-left')
    print(next_state)
    print("agent turned left, orientation should now be north.")
    next_state = example_ghproblem.result(state=example_state, action='turn-right')
    print(next_state)
    print("agent turned right, orientation should now be south.")
    print('________________________________________________________________________')
    
    # health mechanics
    example_state = (0, 0, 'west', 8, 0, False, False)
    print(f'For GridHunterProblem. From state: {example_state}. Taking action: {"stay"}')
    next_state = example_ghproblem.result(state=example_state, action='stay')
    print(next_state)
    print("agent stayed in location with ALIVE monster. health should be 7.")
    example_state = (0, 0, 'west', 8, 0, True, False)
    print(f'For GridHunterProblem. From state: {example_state}. Taking action: {"stay"}')
    next_state = example_ghproblem.result(state=example_state, action='stay')
    print(next_state)
    print("agent stayed in location with DEAD monster. health should remain 8.")
    print()
    
    example_state = (1, 2, 'south', 11, 1, False, False)
    print(f'For GridHunterProblem. From state: {example_state}. Taking action: {"move-forward"}')
    next_state = example_ghproblem.result(state=example_state, action='move-forward')
    print(next_state)
    print("agent walked into location with ALIVE monster. health should be 10.")
    example_state = (1, 2, 'south', 11, 2, False, True)
    print(f'For GridHunterProblem. From state: {example_state}. Taking action: {"move-forward"}')
    next_state = example_ghproblem.result(state=example_state, action='move-forward')
    print(next_state)
    print("agent walked into location with DEAD monster. health should remain 11.")
    print('________________________________________________________________________')
    ...
    example_ghproblem = GridHunterProblem(initial_agent_info=(0, 4, 'west', 10),
                                          N=5, 
                                          monster_coords=[(0,1), (2,2)] )
    goal_node = uniform_cost_search(example_ghproblem)
    ...
    searcher_names= ['Graph-like BFS',  
                     'Graph-like UCS', 
                     'Graph-like A*']
    run_profiler_searches(example_ghproblem, searchers, searcher_names)